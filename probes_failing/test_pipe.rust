...
result1 := 3 | square
put!("3 | square =", result1)
eq!( result1 , 9);

// Pipe with variable
val := 4
result2 := val | double
put!("4 | double =", result2)
eq!( result2 , 8);

// Chained pipes
result3 := 2 | square | double
put!("2 | square | double =", result3)
eq!( result3 , 8);
...
put!("Testing pipe in function args:", increment(5 | negate))

// Test with expressions
exprResult := (2 + 3) | increment
put!("(2 + 3) | increment =", exprResult)
eq!( exprResult , 6);

// Pipe vs bitwise OR distinction
// This should be pipe operator (function call)
pipeResult := 2 | square
put!("2 | square =", pipeResult) // Should be 4
eq!( pipeResult , 4);

// This should be bitwise OR (number | number)
bitwiseResult := 2 | 4
put!("2 | 4 =", bitwiseResult) // Should be 6 (bitwise OR)
eq!( bitwiseResult , 6);

// Mixed operations
temp := 3 | square
mixed := temp | 1
put!("temp = 3 | square =", temp)
put!("temp | 1 =", mixed) // Should be 9 | 1 = 9
eq!( temp , 9);
eq!( mixed , 9);

// Test parenthesized pipe operations
parenResult1 := (3 | square) | 1
put!("(3 | square) | 1 =", parenResult1)
eq!( parenResult1 , 9  ); // 3^2 = 9, 9|1 = 9

parenResult2 := 2 | (4 | square)
put!("2 | (4 | square) =", parenResult2)
eq!( parenResult2 , 18  ); // 4^2 = 16, 2|16 = 18

put!("All pipe operator tests passed!")