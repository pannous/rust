...
impl TokenCursor {
    ...
    /// Fallible [`Self::clone_enclosing_delim`]: `None` rather than a panic when there is no
    /// enclosing delimited sequence, so a malformed token stream can be recovered from.
    #[inline]
    pub fn try_clone_enclosing_delim(&self) -> Option<TokenTree> {
        self.stack.last()?.curr().cloned()
    }
