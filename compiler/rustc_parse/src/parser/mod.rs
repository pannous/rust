...
impl<'a> Parser<'a> {
    ...
    pub fn parse_token_tree(&mut self) -> TokenTree {
        if self.token.kind.open_delim().is_some() {
            ...
            let Some(tree) = self.token_cursor.try_clone_enclosing_delim() else {
                // No enclosing delimited sequence despite an open delimiter: the token stream is
                // malformed (error recovery), so treat the delimiter as a regular token.
