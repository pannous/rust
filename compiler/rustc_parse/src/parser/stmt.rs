    ...
    pub fn parse_stmt_without_recovery(
        ...
        let stmt = if self.token.is_keyword(kw::Super) && self.is_keyword_ahead(1, &[kw::Let]) {
            ...
            if self.is_script_mode() {
                ...
                return Err(self.dcx().create_err(diagnostics::InvalidVariableDeclaration {
                    span: lo,
                    sub: diagnostics::InvalidVariableDeclarationSub::UseLetNotVar(lo),
                }));
            }
            ...
            && !self.is_builtin()
        {
            ...
            false,
            true,
            FnParseMode { req_name: |_, _| true, context: FnContext::Free, req_body: true, in_block: true },
            force_collect,
        ...
        let stmt = self.collect_tokens(None, attrs, ForceCollect::No, |this, attrs| {
            ...
            if this.token == token::ColonEq {
                ...
                if path.segments.len() == 1 && path.segments[0].args.is_none() {
                    ...
                    let (expr, _) =
                        this.parse_expr_res_after_attrs(Restrictions::STOP_AT_NEWLINE, empty_attrs)?;
